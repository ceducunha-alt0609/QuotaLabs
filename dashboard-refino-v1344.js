/* QuotaLab V134.4 — Refino desktop do Dashboard
   - Mantém topbar/sidebar intactos
   - Ajuste percentual junto da cota vigente, sem pílula
   - Índice de Saúde no canto direito do hero
   - Modal montado diretamente no body para abrir centralizado na viewport
   - Não altera a experiência mobile
*/
(() => {
  'use strict';
  if (!window.matchMedia('(min-width: 901px)').matches) return;
  if (window.__QL_DASH_REF_V1344__) return;
  window.__QL_DASH_REF_V1344__ = true;
  ...
  function addStyles(){
    ['ql-dashboard-refino-v134-css','ql-dashboard-refino-v1341-css','ql-dashboard-refino-v1343-css','ql-dashboard-refino-v1344-css'].forEach(id=>{const el=document.getElementById(id);if(el)el.remove();});
    const style=document.createElement('style');
    style.id='ql-dashboard-refino-v1344-css';
    style.textContent=`
      @media (min-width:901px){
        ...
        body > .ql-health-modal{position:fixed!important;inset:0!important;z-index:2147483000!important;display:none!important;align-items:center!important;justify-content:center!important;padding:32px!important;background:rgba(2,8,20,.76)!important;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);}
        body > .ql-health-modal.is-open{display:flex!important;}
        body > .ql-health-modal .ql-health-modal-panel{width:min(1040px,calc(100vw - 64px))!important;max-height:calc(100vh - 64px)!important;overflow:auto!important;position:relative!important;border-radius:22px!important;box-shadow:0 30px 90px rgba(0,0,0,.52)!important;background:var(--bg2)!important;margin:0!important;}
        body > .ql-health-modal .ql-health-modal-close{position:sticky!important;float:right!important;top:12px!important;right:12px!important;z-index:20!important;width:36px!important;height:36px!important;margin:12px 12px -48px 0!important;border-radius:12px!important;border:1px solid var(--border)!important;background:var(--bg3)!important;color:var(--text)!important;display:grid!important;place-items:center!important;font-size:21px!important;line-height:1!important;cursor:pointer!important;box-shadow:0 8px 18px rgba(0,0,0,.16)!important;}
        body > .ql-health-modal .ql-health-modal-content{padding:0!important;clear:both;}
        body > .ql-health-modal .ql-health-modal-copy{display:block!important;visibility:visible!important;opacity:1!important;margin:0!important;padding:24px 18px 18px!important;position:relative!important;transform:none!important;}
        body > .ql-health-modal .ql-health-modal-copy .db-health-main{grid-template-columns:minmax(0,1fr) 132px!important;padding-right:44px!important;}
  ...
  function buildHeroScore(){
    ...
    box.innerHTML='<svg class="ql-health-hero-ring" viewBox="0 0 120 120" aria-hidden="true"><circle cx="60" cy="60" r="48" class="ql-health-hero-ring-bg"></circle><circle cx="60" cy="60" r="48" class="ql-health-hero-ring-val"></circle></svg><div class="ql-health-hero-score-inner"><div class="ql-health-hero-score">—</div><div class="ql-health-hero-status">Calculando</div></div>';
    return box;
  }
  function syncHero(card,hero){
    const s=card.querySelector('#db-health-score'),st=card.querySelector('#db-health-status'),r=card.querySelector('#db-health-ring-val');
    const ds=hero.querySelector('.ql-health-hero-score'),dst=hero.querySelector('.ql-health-hero-status'),dr=hero.querySelector('.ql-health-hero-ring-val');
    if(s&&ds)ds.textContent=s.textContent;
    if(st&&dst)dst.textContent=st.textContent;
    if(r&&dr){const cs=getComputedStyle(r);dr.style.strokeDashoffset=cs.strokeDashoffset||r.style.strokeDashoffset||'301.59';}
    syncTone(card,hero);
  }
  ...
  function init(){
    ...
    if(document.getElementById('ql-health-hero-v1344'))return true;

    addStyles();

    meta.querySelectorAll('.ql-hero-delta-meta').forEach(el=>{if(el!==delta.parentElement)el.remove();});
    if(!delta.parentElement.classList.contains('ql-hero-delta-meta')){
    ...
    const label=document.createElement('div');label.className='ql-health-hero-label';label.textContent='Índice de saúde';
    const hint=document.createElement('div');hint.className='ql-health-hero-hint';hint.textContent='ver análise';
    healthHero.append(label,buildHeroScore(),hint);
    const timestamp=document.getElementById('db-timestamp');
    right.innerHTML='';right.appendChild(healthHero);if(timestamp)right.appendChild(timestamp);

    healthCard.classList.add('ql-health-source-hidden');

    document.querySelectorAll('body > .ql-health-modal').forEach(el=>el.remove());
    const modal=document.createElement('div');
    modal.className='ql-health-modal';
    modal.id='ql-health-modal-v1344';
    ...
    const close=document.createElement('button');close.className='ql-health-modal-close';close.type='button';close.setAttribute('aria-label','Fechar');close.textContent='×';
    const content=document.createElement('div');content.className='ql-health-modal-content';
    panel.append(close,content);modal.appendChild(panel);document.body.appendChild(modal);

    function rebuildModal(){
      content.innerHTML='';
      const copy=healthCard.cloneNode(true);
      copy.classList.remove('ql-health-source-hidden');
      copy.classList.add('ql-health-modal-copy');
      stripIds(copy);
    ...
    healthHero.addEventListener('click',open);
    healthHero.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open();}});
    ...
    console.info('[QuotaLab V134.4] Modal do Índice de Saúde centralizado na viewport.');
    return true;
  }

  function boot(){if(init())return;let t=0;const timer=setInterval(()=>{t++;if(init()||t>80)clearInterval(timer);},125);}
