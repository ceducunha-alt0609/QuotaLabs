...
(() => {
  ...
  function addStyles() {
    const old = document.getElementById('ql-dashboard-refino-v134-css');
    if (old) old.remove();
    const style = document.createElement('style');
    style.id = 'ql-dashboard-refino-v1341-css';
    style.textContent = `
      @media (min-width:901px){
        ...
        #page-dashboard .ql-health-hero{--health-color:#3fb950;--health-rgb:63,185,80;width:126px;min-height:126px;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;border-radius:18px;transition:transform .16s ease,background .16s ease,box-shadow .16s ease;outline:none;position:relative;}
        ...
        #page-dashboard .ql-health-hero-score-inner{text-align:center;position:relative;z-index:1;} #page-dashboard .ql-health-hero-score{font-family:'DM Serif Display','Merriweather',Georgia,serif;font-size:34px;line-height:.95;color:var(--text);font-weight:980;letter-spacing:-.05em;} #page-dashboard .ql-health-hero-status{margin-top:4px;font-size:9px;text-transform:uppercase;letter-spacing:.09em;font-weight:900;color:var(--health-color);} #page-dashboard .ql-health-hero-hint{margin-top:0;font-size:9px;line-height:1;color:var(--text3);opacity:.82;letter-spacing:.02em;}
        #page-dashboard .ql-health-modal{position:fixed;inset:0;z-index:10050;display:none;align-items:center;justify-content:center;padding:34px;background:rgba(2,8,20,.74);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);} #page-dashboard .ql-health-modal.is-open{display:flex!important;}
        ...
        #page-dashboard .ql-health-modal-panel #db-health-card{display:block!important;visibility:visible!important;opacity:1!important;margin:0!important;padding:24px 18px 18px!important;position:relative!important;transform:none!important;}
        #page-dashboard .ql-health-modal-panel #db-health-card .db-health-main{grid-template-columns:minmax(0,1fr) 132px!important;padding-right:44px;} body.ql-health-modal-open{overflow:hidden!important;}
      }
    `;
    document.head.appendChild(style);
  }

  function syncTone(card, hero){const tones=['saude','atencao','alerta','risco'];hero.classList.remove(...tones);hero.classList.add(tones.find(c=>card.classList.contains(c))||'saude');}
  ...
  function init(){
    const page=document.getElementById('page-dashboard'),hero=page&&page.querySelector('.db-hero'),meta=hero&&hero.querySelector('.db-hero-meta'),right=hero&&hero.querySelector('.db-hero-right'),delta=document.getElementById('db-delta-pill'),healthCard=document.getElementById('db-health-card');
    if(!page||!hero||!meta||!right||!delta||!healthCard)return false;
    if(document.getElementById('ql-health-hero-v1341'))return true;
    addStyles();
    ...
    const modal=document.createElement('div');modal.className='ql-health-modal';modal.id='ql-health-modal-v1341';modal.setAttribute('aria-hidden','true');const panel=document.createElement('div');panel.className='ql-health-modal-panel';panel.setAttribute('role','dialog');panel.setAttribute('aria-modal','true');panel.setAttribute('aria-label','Detalhes do Índice de Saúde do Condomínio');const close=document.createElement('button');close.className='ql-health-modal-close';close.type='button';close.setAttribute('aria-label','Fechar');close.textContent='×';panel.append(close,healthCard);modal.appendChild(panel);page.appendChild(modal);syncHero(healthCard,healthHero);
    const open=()=>{syncHero(healthCard,healthHero);modal.classList.add('is-open');modal.setAttribute('aria-hidden','false');document.body.classList.add('ql-health-modal-open');};const shut=()=>{modal.classList.remove('is-open');modal.setAttribute('aria-hidden','true');document.body.classList.remove('ql-health-modal-open');};healthHero.addEventListener('click',open);healthHero.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open();}});close.addEventListener('click',shut);modal.addEventListener('click',e=>{if(e.target===modal)shut();});document.addEventListener('keydown',e=>{if(e.key==='Escape'&&modal.classList.contains('is-open'))shut();});new MutationObserver(()=>syncHero(healthCard,healthHero)).observe(healthCard,{subtree:true,childList:true,characterData:true,attributes:true});
    console.info('[QuotaLab V134.1] Refino desktop carregado por arquivo novo.');return true;
  }
