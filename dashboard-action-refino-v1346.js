...
(() => {
    ...
    style.textContent=`
      @media (min-width:901px){
        #page-dashboard .ql-action-now-btn{
          ...
          border:1px solid rgba(224,170,58,.58)!important;
          background:linear-gradient(180deg,rgba(224,170,58,.30),rgba(224,170,58,.14))!important;
        ...
        #page-dashboard .ql-action-now-btn:focus-visible{
          ...
          box-shadow:0 12px 28px rgba(0,0,0,.20),0 0 0 1px rgba(224,170,58,.12) inset!important;
  ...
  function isTarget(el){
    ...
    return txt==='o que fazer agora?' || txt==='analisar agora';
  }

  function findButton(){
    const direct=document.getElementById('db-decision-btn');
  ...
  function boot(){
    if(refine()) return;
    let tries=0;
    const timer=setInterval(()=>{tries++;if(refine()||tries>120)clearInterval(timer);},125);
  }
